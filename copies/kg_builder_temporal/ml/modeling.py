...
def temporal_train_val_split_indices(
    days: list,
...
def build_day_embedding_frame_from_rolling(
    df_day_nodes: pd.DataFrame,
    day_embedding_dict: Dict[str, np.ndarray],
) -> pd.DataFrame:
    """
    Build embedding frame from rolling day embeddings.
    
    Maps day dates to their aggregated HOPE embeddings computed from
    temporal graph snapshots.
    
    Args:
        df_day_nodes: DataFrame with columns [node_id, day, direction, return_next_day]
        day_embedding_dict: Dict mapping day_date (str) → aggregated_embedding (np.ndarray)
    
    Returns:
        DataFrame with columns [node_id, day, direction, return_next_day, embedding]
    """
    logger.info(
        "Building day embedding frame from %d day nodes and %d day embeddings",
        len(df_day_nodes),
        len(day_embedding_dict),
    )
    ...
    for _, row in df_day_nodes.iterrows():
        day_date = str(row["day"])
        
        if day_date not in day_embedding_dict:
            missing += 1
            logger.debug("Day %s has no embedding in dictionary", day_date)
            continue
        
        embedding = day_embedding_dict[day_date]
    ...
    logger.info(
        "Built day embedding frame: %d rows (%d missing embeddings)",
        len(df),
        missing,
    )
    ...
    if not df.empty:
        ...
        logger.info(
            "Sample embedding shape: %s, dtype: %s",
            sample_emb.shape,
            sample_emb.dtype,
        )
