...
async def build_kg_incremental_candidate(
    ...
    - Stores deterministic text embeddings (text-embedding-3-small) on article nodes
    - Stores publication dates on article nodes for temporal feature computation
    ...
    # Get OpenAI API key for text embeddings
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        logger.warning("OPENAI_API_KEY not set - text embeddings will be skipped")
        ...
        async with semaphore:
            ...
            if api_key:
                try:
                    ...
                    logger.warning("Failed to embed article %s: %s", article_id[:8], str(e))
                    text_embedding = None
