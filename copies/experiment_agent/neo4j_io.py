...
from experiment_incremental_mutator.incremental_kg_mutator import IncrementalArticleKGMutator
...
async def build_kg_for_candidate_incremental(
    driver,
    neo4j_cfg: Neo4jConfig,
    exp_cfg: ExperimentConfig,
    openai_llm: OpenAILLM,
    ...
    """
    Build or extend the knowledge graph for a specific ontology candidate using incremental mutation.
    Reuses existing entities and only adds new relationships instead of rebuilding from scratch.
    
    Args:
        driver: Neo4j driver
        neo4j_cfg: Neo4j configuration
        exp_cfg: Experiment configuration
        openai_llm: LLM for entity extraction (uses Neo4j's LLMInterface)
        openai_embedder: Embeddings model (unused in incremental mode but kept for API compatibility)
        ontology: OntologyCandidate with schema for entity extraction
        articles_df: DataFrame of articles to process
        candidate_tag: Tag to identify this candidate's entities
        accepted_tags: Tags of entities that can be linked to (base + previous best)
    """
    logger.info("Building KG incrementally for ontology candidate: %s", ontology.name)
    
    # Initialize incremental mutator with the provided LLM
    mutator = IncrementalArticleKGMutator(driver=driver, llm=openai_llm)
    
    # Process articles and attach new entities/relationships
    logger.info("Ingesting %d articles incrementally for candidate_tag: %s", len(articles_df), candidate_tag)
    articles_df = articles_df.sort_values("timestamp").reset_index(drop=True)

    # Create a semaphore to limit the number of concurrent tasks
    semaphore = asyncio.Semaphore(50)  # Adjust the limit as needed

    async def limited_mutate_article(*args, **kwargs):
        async with semaphore:
            await mutate_article_incremental(*args, **kwargs)
    ...
    for idx, row in articles_df.iterrows():
        ...
        logger.debug("Scheduling incremental KG mutation for article %d on %s", idx, day)

        # Schedule the task with semaphore
        tasks.append(
            asyncio.create_task(
                limited_mutate_article(
                    mutator,
                    driver,
                    neo4j_cfg,
                    exp_cfg,
    ...
    logger.info("KG for candidate %s updated incrementally with new entities and relationships", candidate_tag)

    # Log the number of nodes and relationships created
    with driver.session(database=neo4j_cfg.database) as session:
        node_count = session.run("MATCH (n) RETURN count(n) AS count").single()["count"]
        relationship_count = session.run("MATCH ()-[r]->() RETURN count(r) AS count").single()["count"]
        logger.info("Ontology candidate '%s' created %d nodes and %d relationships (incremental)", candidate_tag, node_count, relationship_count)
...
async def mutate_article_incremental(
    mutator,
    driver,
    neo4j_cfg,
    exp_cfg,
    ...
    """
    Process a single article asynchronously using incremental mutation.
    """
    # Mutate the article in the graph
    await mutator.mutate_article(
        article_id=f"{exp_cfg.target_ticker}:{idx}",
        text=text,
