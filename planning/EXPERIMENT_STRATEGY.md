...
**Reading effects off the results.** After all 13 chunks complete, pull the 36 job-level MLflow runs (`best_auc`/`best_f1`) and, for any hparam, `groupby(value).best_auc.agg(['mean','std'])`. Group means are directly comparable for lookback / steps / prompt / chain_hops (still pairwise-balanced); ticker contrasts (and any ticker-confounded comparison) should note the 4-vs-5 imbalance. Because the design is only strength-2, a paired/blocked test may block on any **single** other factor (whose margins match) — but **not** on the full joint cell of all remaining factors, which would require a full-factorial or higher-strength design.

**Cross-ticker construct validity (2026-08-05).** MSFT and TSLA are not two samples of the same task — they are two different prediction problems with plausibly different inherent difficulty (per the Ticker Selection rationale above: TSLA "volatile, sentiment-driven" vs MSFT "stable... credible finance benchmark"). **Raw AUC values must not be compared directly across tickers** — `AUC(TSLA, config Y) > AUC(MSFT, config X)` does not imply config Y is better; it may only reflect that TSLA's task has more exploitable signal. **What remains valid: differences of pooled marginal means.** Because `ticker` is balanced (or near-balanced) against every other factor — `ticker×steps` is exactly [6,6], `ticker×lookback/prompt/hops` are ~4-vs-5 — every hparam-level group is averaged over essentially the same MSFT/TSLA mix. A constant per-ticker difficulty offset therefore raises (or lowers) all group means by roughly the same amount and **cancels out when comparing `mean(AUC | hparam=A)` vs `mean(AUC | hparam=B)`**, even though it does not cancel in any single raw AUC value. This is the same confounder-control logic that makes the pairwise-balanced design work at all — ticker-difficulty is simply another lurking variable the balance controls for. **What this design cannot answer**: whether the *best* hparam value differs *between* MSFT and TSLA specifically (an interaction question) — that requires each ticker to have its own independently-balanced sub-design (a 288-config redesign, evaluated and declined as disproportionate to the current 36/13-chunk plan's sunk cost and the marginal-difference approach's sufficiency for the core hypotheses H1-H3/H7b). Per-ticker AUC may still be reported as a **descriptive/exploratory** table, but should be labeled as such, not as a controlled comparison.
