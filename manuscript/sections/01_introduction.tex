...
recommendation to predictive modelling~\citep{ji2021kgcompletion}. The ability to
...
hand-crafted rules~\citep{mo2025kggen}. Despite this progress, a fundamental tension
...
task-based evaluation is feasible, we could not find work
demonstratating use of downstream performance signal to steer KG ontology evolution during the
...
\begin{enumerate}
    ...
    \item[\textit{(iv)}] The resulting task performance score is returned as a feedback signal to choose the best candidate graphs and guide the next ontology revision.
\end{enumerate}
...
findings and limitations, and Section~\ref{sec:conclusion} recapitulates findings.
