...
knowledge distillation. In knowledge graph building three dominant open source providers are LlamaIndex, LangChain, and Neo4j. 
...
performance''~\citep{li2025llmontologyreview}.

Across all reviewed studies, evaluation targets the ontology directly: the metrics in every
case --- precision, recall, F1 --- are applied against ontology benchmarks such as the
Ontology Alignment Evaluation Initiative (OAEI) rather than against a downstream
application.
...
The key design question such approaches leave open is how to define the event schema:
which entity types and relation types should be extracted from news to maximise
predictive utility.
